...
def batch_generator(sequence_list, identifiers, mirna_dict, batch_size):
    batch = []
    for sequence, identifier in zip(sequence_list, identifiers):
        for mirna_id, mirna_seq in mirna_dict.items():
            job = (sequence, mirna_seq, identifier, mirna_id)
            batch.append(job)
...
def prepare_jobs_from_df_batched(df, batch_size=100):
    # Load the MIRNA_CSV file
    mirna_df = pd.read_csv(MIRNA_CSV)
    ...
    # Create batched generators for wild-type and mutated jobs
    wt_job_generator = batch_generator(wt_sequences, identifiers, mirna_dict, batch_size)
    mutated_job_generator = batch_generator(mutated_sequences, identifiers, mirna_dict, batch_size)

    return wt_job_generator, mutated_job_generator

def run_rnaduplex_batched(batch):
    results = []
    for long_sequence, short_sequence, long_identifier, short_identifier in batch:
        # Handle the sequence input
        input_sequence = f"{long_sequence}\n{short_sequence}"
        ...
        try:
            ...
            results.append((start_long - 1, end_long, long_bracket, start_short - 1, end_short, short_bracket, energy_value, long_identifier, short_identifier, long_sequence, short_sequence))

        except ValueError as e:
            # Handle cases where the output format is unexpected
            lines = result.stdout.split("\n")
            if first_line := lines[0].split():
                brackets, long_start_end, _, short_start_end, _, energy = first_line
                long_bracket, short_bracket = brackets.split("&")
                start_long, end_long = map(int, long_start_end.split(","))
                start_short, end_short = map(int, short_start_end.split(","))
                ...
                results.append((0, 1, ".", 0, 1, ".", 0, long_identifier, short_identifier, long_sequence, short_sequence))
    return results

def run_jobs_async_batched(job_generator, binary_value, verbose=False, num_workers=cpu_count()):

    # Configure logging
    logging.basicConfig(level=logging.INFO if verbose else logging.WARNING)
    
    with ProcessPoolExecutor(max_workers=num_workers) as executor:
        # Submit batches of jobs to the executor
        futures = {executor.submit(run_rnaduplex_batched, batch): batch for batch in job_generator}
        results = []
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                # Extend the results list with the results of the current batch
                for result in batch_results:
                    result_with_binary = result + (binary_value,)
                    results.append(result_with_binary)
                if verbose:
                    timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
                    logging.info(f"{timestamp} - Completed batch: {batch}")
